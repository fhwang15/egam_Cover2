...
public class Routine : MonoBehaviour
{
    ...
    //Detect the Fishing spot (default)
    /// Raycast or OnTriggerEnter 
    /// If the player is within the 범위, 그러면 fishing rod를 던질 기회 자체가 주어진다. canThrow bool이 true가댐

    //Throw the fishing rod to the fishing spot (Distance determined by how long you press) (casting) (Default)
    ...
    //Wait for the fish to get caught (Bobbing) (Fishing)
    ///rodIsThrown state에 fishing state면 물고기새끼가 잡힐때까지 기다려야함
    ///만약 n초이상 안 잡히면 실패. (stretch) 당장은 성공 케이스만 넣기

    //The Fish will be caught (Single type of fish for now) (hooked) (Fishing)
